...
import {
  FireFilled, FireOutlined, CommentOutlined, PlusOutlined, UnlockOutlined,
  MoreOutlined, DollarOutlined, LockOutlined, FlagOutlined,
  FileImageOutlined, VideoCameraOutlined, EyeOutlined
...
import {
  reactionService, feedService, tokenTransctionService, paymentService, reportService, performerService
} from '@services/index';
import {
  Avatar, Button, Divider, Dropdown, Image, Menu, Modal, Progress, Tooltip, message
} from 'antd';
...
class FeedCard extends Component<IProps> {
  ...
  async handleSendTipCrypto(
    ...
    if(ticker == "ICP"){
      ...
      if(Number(balICRC1) < Number(amountToSend) + 10){
        ...
        this.setState({
          requesting: false, submiting: false, openTipProgressModal: false, tipProgress: 0
        });
      }
  ...
  render() {
    ...
    let canView = (feed?.isSale === 'subscription' && (feed?.isSubscribed || fromExplore === true))
    ...
    return (
      <div className={styles.componentsPostPostCardModule}>
        <div className="feed-card" style={{margin: isPostDetails ? 'auto' : ''}}>
          <div className="feed-top">
            ...
            >
              <div className="feed-top-left">
                <Avatar className="ant-avatart-image" alt="per_atv" src={performer?.avatar || '/static/no-avatar.png'} size={40} style={{ minHeight: '40px', minWidth: '40px' }} />
                {participants?.length === 2 && (<Avatar alt="per_atv" className="participant-avatar-feed" src={participants[1].avatar || '/static/no-avatar.png'} size={40} style={{ minHeight: '40px', minWidth: '40px' }} />)}
          ...
          <div className="post-container">
            ...
            <div className="lock-content">
              ...
              <div className="lock-middle">
                {(isHovered) ? <FontAwesomeIcon icon={faLockOpen} className='lock-icon'/> : <FontAwesomeIcon icon={faLock} className='lock-icon'/>}
                ...
                <Button
                  ...
                  disabled={user.isPerformer}
                  className='lock-btn'
                  onClick={() => Router.push({ pathname: `/artist/profile?id=${performer?.username || performer?._id}` }, `/artist/profile?id=${performer?.username || performer?._id}`)}
                >
                  Follow for free
                </Button>
                )}
                {feed?.teaser && (
                <EyeOutlined className="teaser-btn" type="link" onClick={() => this.setState({ openTeaser: true })} style={{ marginTop: '10px' }} />
                )}
              </div>
          ...
          >
            <TipPerformerForm user={user} isProfile={false} performer={performer} participants={participants} submiting={requesting} onFinish={this.sendTip.bind(this)} />
          </Modal>
